...
class DivingGroupForm extends Form {
  constructor(props) {
    super(props);
    this.state = {
      data: {
  ...
  schema = {
    heading: Joi.string().required().label("Diving Group Head"),
    body: Joi.string().required().label("Diving Group Body"),
    image: Joi.string().allow("").optional().label("Diving Group Image"),
    caption: Joi.string().allow("").optional().label("Caption"),
    video: Joi.string().allow("").optional().label("Diving Group video"),
    videoCaption: Joi.string().allow("").optional().label("videoCaption"),
  };
  async componentDidMount() {
    await this.populateDivingGroup();
  }
  async populateDivingGroup() {
    try {
      const divingGroupId = this.props.match.params.id;
      if (!divingGroupId) return;

      const { data: divingGroup } = await getDivingGroup(divingGroupId);
      const _id = divingGroupId ? divingGroupId : "";
      this.setState({ _id, data: this.mapToViewModelDynamic(divingGroup) });
    } catch (ex) {}
  }
  mapToViewModelDynamic(divingGroup) {
    return {
      _id: divingGroup._id,
      heading: divingGroup.heading,
      body: divingGroup.body,
      image: divingGroup.image,
      caption: divingGroup.caption,
      video: divingGroup.video,
      videoCaption: divingGroup.videoCaption,
    };
  }
  doSubmit = async () => {
    const response = await saveDivingGroup(this.state.data);
          ...
          <div className="row admin-form">
            ...
            <div className="col-md-7">
              ...
              <div className="mainform-elements">
                <h1>Add divingGroup page sub page articles</h1>
                <TnyEditor
                  heading={heading}
                  value={heading}
...
export default DivingGroupForm;
