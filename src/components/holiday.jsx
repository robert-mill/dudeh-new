...
import {getHolidayGallerys, deleteHolidayGallery} from "../services/holidayGalleryService";
...
class Holiday extends Component {
  ...
  async componentDidMount() {
    ...
    const { data: groupData } = await getHolidayGroups();
    const {data: gallery} = await getHolidayGallerys();
    const { data } = [];
  ...
  handleDelete = async (holiday) => {
    ...
    const data = originaldata.filter((m) => m._id !== holiday._id);
    this.setState({ data });
    try {
  ...
  handleGroupItemDelete = async (holidayGroup) => {
    const originaldata = this.state.data;
    const data = originaldata.filter((m) => m._id !== holidayGroup._id);
    this.setState({ data });
    try {
      await deleteHolidayGroup(holidayGroup._id);
    } catch (ex) {
      if (ex.response && ex.response.status === 404)
        toast.error("This data has already been deleted");
      this.setState({ data: originaldata });
  ...
  handleGalleryDelete = async (g) =>{
    ...
    try {
      await deleteHolidayGallery(g._id);
    } catch (ex) {
      if (ex.response && ex.response.status === 404)
          ...
          <React.Fragment>
            <div className="row">
              <button className="btn btn-primary">
                <Link className="navbar-brand" to={`/holiday/new`}>
                  New main article
                </Link>
              </button>
              <button className="btn btn-primary">
                <Link className="navbar-brand" to={`/holidayGroup/new`}>
                  New sub panal
                </Link>
              </button>
              <button className="btn byn-secondary">
                <Link className="navbar-brand" to={`/holidayGallery/new`}>Add gallery pic</Link>
              </button>
            </div>
          </React.Fragment>
        )}
            ...
            <div className="row">
              {newArr && (
                <div className="col-lg-2 col-md-12 article-index">
                  <div className="about-pan-head">Holiday articles</div>
              ...
              <div className={`col-lg-${(groupData && "7 gallery-block")  || "9 gallery-block"} col-md-12 gallery-block`}>
                ...
                {(
                  ...
                  (
                  <React.Fragment>
                    {/* <Gallery3D gallery={ filtered} name={name} className="gallerydesktop" page="holidayGallery"/> */}
                    <div  className="gallerymobile row"><div className="col-12"><GallerySimple  data={filtered} name={name} onChange={this.onChange} onDelete={this.handleGalleryDelete}/></div></div>
                  </React.Fragment>
