...
import { saveHoliday, getHoliday } from "../services/holidayService";
...
class HolidayForm extends Form {
  ...
  schema = {
    heading: Joi.string().required().label("Holiday Head"),
    body: Joi.string().required().label("Holiday Body"),
    image: Joi.string().allow("").optional().label("Holiday Image"),
    caption: Joi.string().allow("").optional().label("Caption"),
    video: Joi.string().allow("").optional().label("Holiday Video"),
  ...
  async populateHoliday() {
    try {
      ...
      const { data: holiday } = await getHoliday(holidayId);
  ...
  mapToViewModelDynamic(holiday) {
    return {
      ...
      video: holiday.video,
      videoCaption: holiday.videoCaption,
    };
  }
  doSubmit = async () => {
    const response = await saveHoliday(this.state.data);
          ...
          <div className="row admin-form">
            ...
            <div className="col-md-7">
              ...
              <div className="mainform-elements">
                <h1>Add holiday page main details</h1>
                <TnyEditor
                  heading={heading}
                  value={heading}
