...
import {
  saveAboutGallery,
  getAboutGallery,
} from "../services/aboutGalleryService";
import auth from "../services/authService";
import TnyEditor from "./common/editorInput";
import PhotoUpload from "./common/photoUpload";

class AboutGalleryForm extends Form {
  constructor(props) {
    super(props);
    this.state = {
      data: {
  ...
  async componentDidMount() {
    await this.populateAboutGallery();
  }
  async populateAboutGallery() {
    try {
      const aboutId = this.props.match.params.id;
      if (!aboutId) return;

      const { data: about } = await getAboutGallery(aboutId);
  ...
  doSubmit = async () => {
    const response = await saveAboutGallery(this.state.data);
