...
import { saveMath, getMath } from "../services/mathsService";
import auth from "../services/authService";
import TnyEditor from "./common/editorInput";
import PhotoUpload from "./common/photoUpload";

class MathsForm extends Form {
  constructor(props) {
    super(props);
    this.state = {
      data: {
  ...
  schema = {
    heading: Joi.string().required().label("About Head"),
    body: Joi.string().required().label("About Body"),
    image: Joi.string().allow("").optional().label("About Image"),
  ...
  async populateAbout() {
    try {
      const mathId = this.props.match.params.id;
      if (!mathId) return;

      const { data: math } = await getMath(mathId);
      const _id = mathId ? mathId : "";
      this.setState({ _id, data: this.mapToViewModelDynamic(math) });
    } catch (ex) {}
  }
  mapToViewModelDynamic(math) {
    return {
      _id: math._id,
  ...
  doSubmit = async () => {
    const response = await saveMath(this.state.data);
          ...
          <div className="row admin-form">
            ...
            <div className="col-md-7">
              ...
              <div className="mainform-elements">
                <h1>Add math page main details</h1>
                <TnyEditor
                  heading={heading}
                  value={heading}
                  label="About Heading"
